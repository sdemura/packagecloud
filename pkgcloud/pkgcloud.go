...
// ErrFilenameAlreadyTaken is a sentinel error indicating that there is already
// a file with that n ame.
...
const userAgent = "packagecloud Go client"
...
func NewClient(token string) (*Client, error) {
	...
	endpointURL := os.Getenv("PACKAGECLOUD_API_URL")
	if endpointURL == "" {
		endpointURL = EndpointURL
	}

	return &Client{token: token, endpointURL: endpointURL, progressBar: false}, nil
}
	...
	case http.StatusUnprocessableEntity: // Unprocessable Entity
		...
		for _, messages := range v {
			for _, msg := range messages {
				...
				if msg == "has already been taken" {
					return ErrFilenameAlreadyTaken
				}
...
func (c Client) CreatePackage(ctx context.Context, repo, distro, pkgFile string) error {
	...
	endpoint := fmt.Sprintf("%s/repos/%s/packages.json", c.endpointURL, repo)
	req, err := NewUploadRequest(endpoint, extraParams, "package[package_file]", pkgFile)
	if err != nil {
		return err
	}
...
func (c Client) All(ctx context.Context, repo string) ([]Package, error) {
	endpoint := fmt.Sprintf("%s/repos/%s/packages.json", EndpointURL, repo)
	return c.paginatedRequest(ctx, http.MethodGet, endpoint)
}
...
func (c Client) Destroy(ctx context.Context, repo, packageFilename string) error {
	pkg := filepath.Base(packageFilename)

	endpoint := fmt.Sprintf("%s/repos/%s/%s", EndpointURL, repo, pkg)
	_, err := c.apiRequest(ctx, http.MethodDelete, endpoint, &struct{}{})
...
func (c Client) Search(ctx context.Context, repo, q, filter, dist string, perPage int) ([]Package, error) {
	endpoint := fmt.Sprintf("%s/repos/%s/search.json?q=%s&filter=%s&dist=%s&per_page=%d", EndpointURL, repo, q, filter, dist, perPage)
	return c.paginatedRequest(ctx, http.MethodGet, endpoint)
}

func (c Client) paginatedRequest(ctx context.Context, method string, endpoint string) ([]Package, error) {
	var (
		allPackages []Package
		getLastPage bool
	)
	...
	for {
		...
		group, err := c.apiRequest(ctx, method, endpoint, &pkgs)
		if err != nil {
			return nil, err
		}
...
func (c Client) apiRequest(ctx context.Context, method string, endpoint string, decodedResp interface{}) (link.Group, error) {
